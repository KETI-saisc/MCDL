...
class TinySleepNet(object):
    ...
    def __init__(
        ...
        config,
        output_dir="./output",
        use_rnn = False, ## true ###################### for id (need to be checked to false)
        testing = False, ## False
        ...
        #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!use_rnn :", use_rnn)##########################################original

        # Placeholder
        with tf.variable_scope("placeholders") as scope:
            self.signals = tf.placeholder(dtype=tf.float32, shape=(None, self.config["input_size"], 1, 1), name='signals')  ########original
            # self.signals = tf.placeholder(dtype=tf.float32, shape=(None, self.config["input_size"], 2, 1), name='signals') ################ 채널 2개 받기 위해#############################
            # self.signals = tf.placeholder(dtype=tf.float32, shape=(None, self.config["input_size"], 13, 1), name='signals')  ################ 채널 13개 받기 위해##########################
            # self.signals = tf.placeholder(dtype=tf.float32, shape=(None, self.config["input_size"], 3, 1), name='signals')  ################ 채널 3개 받기 위해############################
            # self.signals = tf.placeholder(dtype=tf.float32, shape=(None, self.config["input_size"], 4, 1), name='signals')  ################ 채널 4개 받기 위해############################
        ...
        #print("initial satate!!!!!", self.init_state)#############################################################################추가
        #print("Final state!!!!!!!!", self.final_state)#############################################################################추가
        ...
        if use_best:
            if os.path.exists(self.best_ckpt_path):
                if os.path.isfile(os.path.join(self.best_ckpt_path, "checkpoint")):
                    ...
                    self.saver.restore(self.sess, latest_checkpoint) ###########################################모델 복원
    ...
    def train(self, minibatches):
        ...
        if not self.use_rnn: ### False
            ...
            #for x, y in minibatches: ################################################################################### original
            for x, y, w, sl, re in minibatches:
                ...
                # preds.extend(outputs["train/preds"]) ### original
                # trues.extend(y) ### original
...
############################################################################################################################################# end for id
        ...
        cm = skmetrics.confusion_matrix(y_true=trues, y_pred=preds, labels=range(self.config["n_classes"]))  ######################## for id
    ...
    def evaluate(self, minibatches):
        ...
        preds = []
        trues = []
        soft_test = []  ################################################################### 추가(prediction할때 쓰려고)

        if not self.use_rnn: ### True for ecg identification ### false for sleep classification
            #for x, y in minibatches: ## original
            print(time.strftime('b-0 :' + '%H-%M-%S'))
            for x, y, w, sl, re in minibatches: ######################
                feed_dict = {
                    self.signals: x,
                    self.labels: y,
...
############################################################################################################################################# strat for id
                ...
                print(time.strftime('b-2 :' + '%H-%M-%S'))

                for i in range(self.config["batch_size"]):
                    preds.extend(tmp_preds[i, :sl[i]])
                    trues.extend(tmp_trues[i, :sl[i]])

                print(time.strftime('b-3 :' + '%H-%M-%S'))
############################################################################################################################################## end for id


        else: ################## True
            print(time.strftime('a-1 :' + '%H-%M-%S'))
            for x, y, w, sl, re in minibatches:
                feed_dict = {
                    self.signals: x,
                    self.labels: y,
                ...
                outputs = self.run(self.test_outputs, feed_dict=feed_dict)
                print(time.strftime('a-2 :' + '%H-%M-%S'))

                ################################################################### 추가(prediction할때 쓰려고)
#########################################################################################################
                #print("$%$%$%$%$%$%$%$%$%$%$%4")
                #print(self.run(tf.nn.softmax(self.logits), feed_dict=feed_dict))
                #print(type((self.run(tf.nn.softmax(self.logits), feed_dict=feed_dict))[0]))

                # print("$333333333333333333333")
                # print(tf.argmax(self.logits, axis=1))

                soft_test.extend(self.run(tf.nn.softmax(self.logits), feed_dict=feed_dict))  ############
                print(time.strftime('a-3 :' + '%H-%M-%S'))
############################################################################################################
        ...
        outputs = {
            ...
            "test/softmax": soft_test,####################################################### 추가(prediction할때 쓰려고)
        }
        return outputs
